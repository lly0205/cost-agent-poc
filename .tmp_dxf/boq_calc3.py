...
# columns by COLU polygon centroid in S5, name by nearest 柱集中标注
labels = []
for e in msp.query("TEXT"):
    if e.dxf.layer == "柱集中标注":
        x, y = e.dxf.insert.x, e.dxf.insert.y
        if X0 <= x <= X1 and band(y) == "S5":
            labels.append((x, y, re.sub(r"\(.*\)", "", e.dxf.text).strip()))
colu5 = []
for e in msp.query("LWPOLYLINE"):
    if e.dxf.layer == "COLU":
        pts = [(p[0], p[1]) for p in e.get_points("xy")]
        cx = sum(p[0] for p in pts)/len(pts); cy = sum(p[1] for p in pts)/len(pts)
        if X0 <= cx <= X1 and band(cy) == "S5":
            colu5.append((cx, cy))
print("COLU in S5:", len(colu5))
HZ = {"DEEP": 6.22, "SHALLOW": 4.52, "PERIM": 0.22}
zone_res = collections.defaultdict(collections.Counter)
for cx, cy in colu5:
    p = Point(cx, cy)
    zone = "DEEP" if hatch5.buffer(300).contains(p) else ("SHALLOW" if enc5.buffer(300).contains(p) else "PERIM")
    best, bd = None, 1e18
    for lx, ly, nm in labels:
        d = (lx-cx)**2+(ly-cy)**2
        if d < bd: bd, best = d, nm
    zone_res[zone][best] += 1
print("\n=== columns by geometry ===")
for z in ("DEEP", "SHALLOW", "PERIM"):
    n = v = f = 0
    parts = []
    for nm, c in sorted(zone_res[z].items()):
        b, h = SEC.get(nm, (550, 550))
        n += c; v += c*b/1000*h/1000*HZ[z]; f += c*2*(b+h)/1000*HZ[z]
        parts.append(f"{nm}({b}x{h})x{c}")
    print(f"{z}: n={n} H={HZ[z]} vol={v:.2f} m3 fw={f:.1f} m2")
    print("   ", "; ".join(parts))

# beams with 3-way: in / edge(<=600 of boundary) / out
def pair_lines(items_raw, horiz):
    ...
    for i in range(len(items)):
        ...
        for j in range(i+1, len(items)):
            ...
            if ov1-ov0 > 600: out.append(((ci+cj)/2, round(w,-1), ov0, ov1))
    return out
def dedupe(strips):
...
for e in msp.query("TEXT"):
    if e.dxf.layer == "梁名称编号":
        ...
        if X0 <= x <= X1 and band(y) == "S6":
            t = e.dxf.text.strip(); parts = t.split()
            if len(parts) > 1: name2sec[parts[0]] = parts[1]
...
for strips, horiz in ((hs, True), (vs, False)):
    for c, w, a0, a1 in strips:
        ...
        Lin = net.intersection(enc_in).length/1000
        ...
        key = ("KL" if best[0].startswith("KL") else "L", best[1]) if best else ("UNK", f"{int(w)}x?")
        agg[key][0] += Lin; agg[key][1] += Lout

print("\n=== beams: in(顶板梁含边梁) / out(基础梁) ===")
ti = to = vin = vout = vrib = fsi = fso = bbot = vfd = 0
for k in sorted(agg, key=str):
    Lin, Lout = agg[k]
    try: b, h = (int(v) for v in k[1].split("x"))
    except Exception: b, h = int(k[1].split("x")[0]), 700
    vi = Lin*b/1000*h/1000; vo = Lout*b/1000*h/1000
    if Lin > 0.05 or Lout > 0.05:
    ...
    vfd += (b+0.2*1000)/1000*Lout*0.15  # 基础梁垫层
print(f"TOTALS: len in={ti:.1f}m out={to:.1f}m | vol full in={vin:.1f} out={vout:.1f} | rib(in)={vrib:.1f}")
print(f"fw: in_sides={fsi:.1f} m2, in_bottoms={bbot:.1f} m2, out_2sides={fso:.1f} m2; 基础梁垫层 vol={vfd:.1f} m3")
