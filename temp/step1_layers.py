...
t1 = time.time()
print(f"读取完毕，总行数: {len(lines)}，耗时: {t1-t0:.1f}s")
...
print(f"ENTITIES区段: 行{s} ~ 行{e}，共 {e-s} 行")

# 提取所有图层名（ENTITIES段内，组码8后的行）
layers = set()
...
# 还要从TABLES/LAYER段提取完整图层列表
print("\n========== 从TABLES中提取图层 ==========")
table_layers = set()
in_layer_table = False
for i in range(len(lines)-3):
    # 找LAYER表
    if lines[i].strip()=="0" and lines[i+1].strip()=="TABLE" and lines[i+2].strip()=="2" and lines[i+3].strip()=="LAYER":
        in_layer_table = True
    if in_layer_table:
        if lines[i].strip()=="0" and lines[i+1].strip()=="ENDTAB":
            in_layer_table = False
        if lines[i].strip()=="0" and lines[i+1].strip()=="LAYER":
            # 读取图层名（组码2）
            for j in range(i+2, min(i+20, len(lines)-1)):
                if lines[j].strip()=="2":
                    table_layers.add(lines[j+1].strip())
                    break

print(f"TABLES中图层数: {len(table_layers)}")
